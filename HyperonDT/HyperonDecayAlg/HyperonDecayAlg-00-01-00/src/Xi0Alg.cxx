...
StatusCode Xi0Alg::execute()
{
    ...
    if (m_run % 10000 == 0)
        log << MSG::WARNING << "run, evtnum = " << m_run << " , " << m_event << endreq;

    if (m_saveMcTruth && eventHeader->runNumber() < 0)
        mc_truth();
    ...
    //if (totalCharged == 0 || totalCharged > 12)
    if (totalCharged == 0 || totalCharged > 200)
        return StatusCode::SUCCESS;
    Num[1] += 1;
    ...
    //if (nphoton > 10) //I have removed this becaue no need to restrictnumber of photon
      //  return StatusCode::SUCCESS;
    std::vector<int> iGoodGam;
    iGoodGam.clear();
    myTools->selectNeutralTracks(evtRecEvent, evtRecTrkCol, iGoodGam);
    //if (iGoodGam.size() < 2 || iGoodGam.size() > 10)
    if (iGoodGam.size() < 2) //minimum two photons needed for pi0 and no need for restrict photons
        return StatusCode::SUCCESS;
    ...
    for (int i = 0; i < iGoodGam.size(); i++)
    {
        ...
        for (int j = i + 1; j < iGoodGam.size(); j++)
        {
            ...
            if (mgg_this < 0.11 || mgg_this > 0.15)
                continue;
    ...
    for (int c = 0; c < totalCharged; c++)
    {
        ...
        if (trkId1_this == id1_best || trkId1_this == id2_best)
            continue;
        for (int d = c + 1; d < totalCharged; d++)
        {
            EvtRecTrackIterator itTrk2 = evtRecTrkCol->begin() + d;
            int trkId2_this = (*itTrk2)->trackId();
            if (trkId2_this == id1_best || trkId2_this == id2_best)
                continue;
            ...
            for (int ll = 0; ll < 4; ll++)
                m_p4otherLambda[notherLambda][ll] = p4candidate[ll];
    ...
    for (int a = 0; a < iGood.size(); a++)
    {
        ...
        if (myTools->isGoodElectron(itTrk))
        {
            ...
            for (int ll = 0; ll < 4; ll++)
                m_p4e[ne][ll] = mdcKalTrk->p4(myTools->getMassE())[ll];
...
void Xi0Alg::mc_truth()
{
    ...
    {
        if ((*iter_mc_topo)->primaryParticle() && (*iter_mc_topo)->particleProperty() == 11 && ((*iter_mc_topo)->mother()).particleProperty() == 11)
        {
            strange = true;
        }
        if ((*iter_mc_topo)->primaryParticle())
            continue;
        if (!(*iter_mc_topo)->decayFromGenerator())
            continue;
        ...
        if (strange && ((*iter_mc_topo)->mother()).particleProperty() != 443)
            mcidx--;
        HepLorentzVector p4iter = (*iter_mc_topo)->initialFourMomentum();
        for (int a = 0; a < 4; a++)
            m_p4truth[m_numParticle][a] = p4iter[a];
