...
def mass_diff_pi0(chain):
    ...
    for l in range(chain.ngshw):
        for m in range(chain.ngshw):
            ...
            p4shw_gam1 = ROOT.TLorentzVector(
                chain.p4shw[indexgshw1], chain.p4shw[indexgshw1+1], chain.p4shw[indexgshw1+2], chain.p4shw[indexgshw1+3])
            p4shw_gam2 = ROOT.TLorentzVector(
                chain.p4shw[indexgshw2], chain.p4shw[indexgshw2+1], chain.p4shw[indexgshw2+2], chain.p4shw[indexgshw2+3])
    ...
    if tempindexgshw2 != -1 and tempindexgshw1 != -1:
        p4shw_gam1 = ROOT.TLorentzVector(
            chain.p4shw[tempindexgshw1], chain.p4shw[tempindexgshw1+1], chain.p4shw[tempindexgshw1+2], chain.p4shw[tempindexgshw1+3])
        p4shw_gam2 = ROOT.TLorentzVector(
            chain.p4shw[tempindexgshw2], chain.p4shw[tempindexgshw2+1], chain.p4shw[tempindexgshw2+2], chain.p4shw[tempindexgshw2+3])
        p4shw_gam12 = p4shw_gam1 + p4shw_gam2
...
def mass_loop_pi0(chain):
    #print('\n')
    p4_lambda = ROOT.TLorentzVector(
        chain.p4lambda[0], chain.p4lambda[1], chain.p4lambda[2], chain.p4lambda[3])
...
def main():
    ...
    for k in range(entries):
        ...
        mass_lambda = p4_lambda.M()
        mlambda[0] = mass_lambda
        if (mlambda[0] < 1.110683 or mlambda[0] > 1.120683):
            continue
        ...
        p4shw_gam1 = ROOT.TLorentzVector(
            chain.p4shw[Idgam1*6], chain.p4shw[Idgam1*6+1], chain.p4shw[Idgam1*6+2], chain.p4shw[Idgam1*6+3])
        p4shw_gam2 = ROOT.TLorentzVector(
            chain.p4shw[Idgam2*6], chain.p4shw[Idgam2*6+1], chain.p4shw[Idgam2*6+2], chain.p4shw[Idgam2*6+3])
        p4shw_gam12 = p4shw_gam1 + p4shw_gam2
        mass_gam12 = p4shw_gam12.M()
        mgamgam[0] = mass_gam12
        # p4_xi0 = p4shw_gam12 + p4_lambda
        ...
        # mass_xi0 = p4_xi0.M()
        mxi0[0] = mass_xi0
        if (mxi0[0] < 1.304 or mxi0[0] > 1.324):
            continue
        # p4_rec_xi0 = cms_p4 - p4_xi0
        # rec_mass_xi0 = p4_rec_xi0.M()
